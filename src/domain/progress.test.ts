...
describe("progress helpers", () => {
  it("counts completed checklist items, completed sets, and weekly estimated minutes", () => {
    const tue = getPlanForWeekday("tue");
    expect(getRecordCompletion(tue, { completedItemIds: ["coach-class", "shadow"] }).completed).toBe(2);
    expect(getRecordCompletion(tue, { completedItemIds: [], itemSetLogs: { "heavy-bag": [{ id: "set-1", completed: true }] } }).completed).toBe(1);
